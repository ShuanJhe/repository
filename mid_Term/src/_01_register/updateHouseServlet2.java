...
@WebServlet("/upadte2")
public class updateHouseServlet2 extends HttpServlet {
		...
		if (num == null || num.trim().length() == 0) {
			...
			try {
				...
				if (beanCheck != null) {	
					...
					errorMessage.put("cW","查無此資料");	
					RequestDispatcher rd = request.getRequestDispatcher("/index.jsp");
					rd.forward(request, response);
					return;
				}
			} catch (NumberFormatException e) {
				errorMessage.put("cW","格式錯誤");
				RequestDispatcher rd = request.getRequestDispatcher("/index.jsp");
				rd.forward(request, response);
				return;
			} catch (SQLException e) {
				errorMessage.put("cW","資料庫存取錯誤");
				RequestDispatcher rd = request.getRequestDispatcher("/index.jsp");
				rd.forward(request, response);
				return;
			} 
